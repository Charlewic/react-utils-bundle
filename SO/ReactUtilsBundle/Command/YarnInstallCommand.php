...
class YarnInstallCommand extends Command
{
    ...
    {
        $this
            ->setName('soreact_utils:yarn:install')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $output->writeln('Start Yarn install in ['.$bundlePath.'] folder');
        $output->writeln('');

        $process = new Process('(cd \''.$bundlePath.'\' && \''.$this->yarnBinPath.'\' install)');
        ...
        $output->writeln('');
        $output->writeln('Yarn install Successful');
    }
}
